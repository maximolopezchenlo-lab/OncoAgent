...
### [Hito UI: Optimización de Streaming Asíncrono] - 2026-05-08
*   **Problema:** El dashboard clínico se bloqueaba y daba timeout frecuentemente durante el procesamiento del RAG con LangGraph, generando una mala experiencia ("página trabada").
*   **Decisión Arquitectónica:** Se convirtió el handler `process_and_update` de Gradio en un generador sincrónico (`yield`).
*   **Enfoque Lógico:** Al hacer `yield` de un estado inicial de "Cargando" en todos los componentes de salida antes de ejecutar la función pesada `run_triage`, Gradio mantiene viva la conexión HTTP y proporciona feedback visual inmediato.
*   **Métricas de Rendimiento:** Cero timeouts del navegador durante el triaje. La latencia percibida bajó drásticamente gracias al feedback instantáneo en la UI al hacer clic en "Send".

### [Hito UI: Reescritura de Copilot Conversacional estilo ChatGPT] - 2026-05-08
*   **Problema:** La interfaz de triaje de un solo disparo no soportaba conversaciones clínicas iterativas. Los oncólogos necesitaban la capacidad de hacer preguntas de seguimiento, refinar sus consultas y mantener el contexto entre múltiples interacciones — análogo a cómo operan los asistentes de IA modernos (ChatGPT, Gemini, Claude).
...
### El Problema
El lanzamiento de Gradio 6 introdujo cambios disruptivos en el componente , específicamente eliminando el argumento  y estandarizando un nuevo formato de  (Lista de Diccionarios) en lugar del formato heredado de  (Lista de Listas). Intentar ejecutar el dashboard con  resultó en un , y la interfaz no lograba renderizar el historial de conversación.

### Justificación de Decisión Arquitectónica
Para mantener la compatibilidad futura y aprovechar las mejoras de rendimiento de Gradio 6, migramos completamente la gestión del estado de la UI al nuevo esquema de . Esto se alinea con el estándar de OpenAI/Anthropic para el historial de chat ().

### Enfoque Lógico/Técnico
1. **Migración de Esquema:** Se refactorizó  para inicializar y actualizar  como una . 
2. **Actualización de Lógica de Streaming:** Se modificó  para añadir y actualizar correctamente el contenido del mensaje usando la clave  en lugar de índices de lista ().
3. **Limpieza de Parámetros:** Se eliminó el argumento no soportado  del constructor de .
4. **Resiliencia:** Se implementó un script de limpieza robusto con  para resolver conflictos de puertos (7860) durante el despliegue iterativo del nuevo backend de Gradio 6.
