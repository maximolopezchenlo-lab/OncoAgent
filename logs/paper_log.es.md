...
- **Problema/Hipótesis:** El mecanismo anti-alucinación de la Puerta de Distancia requiere un umbral preciso para separar las consultas médicas relevantes de las entradas fuera de dominio.
- **Justificación Arquitectónica:** Creamos un script de calibración (`rag_engine/test_threshold.py`) para evaluar sistemáticamente las distancias del bi-encoder. Las consultas médicas puntuaron consistentemente entre ~0.06-0.09, mientras que las consultas no médicas puntuaron entre ~0.11-0.15.
- **Implementación Lógica/Técnica:** Establecimos el `distance_threshold` estricto en `rag_engine/retriever.py` a `0.10`. Esto actúa efectivamente como una barrera de seguridad estricta: cualquier consulta que resulte en embeddings más lejanos a 0.10 es automáticamente rechazada antes de siquiera alcanzar al LLM, garantizando cero alucinaciones para entradas fuera de dominio.

## Hito: Manual Integral de Marca
**Fecha:** 2026-05-05
**Estado:** Completado

- **Problema/Hipótesis:** Al transicionar OncoAgent de un prototipo de ingeniería a una presentación de hackathon, la falta de una identidad visual y comunicativa unificada arriesgaba un mensaje fragmentado en redes sociales, presentaciones y documentación. Sin estándares de marca codificados, cada nuevo asset (slides, posts, diagramas) introduciría inconsistencias que socavan la credibilidad profesional.
- **Justificación Arquitectónica:** Se creó un manual de marca integral (`docs/brand_guidelines.md`) cubriendo 12 secciones: Esencia de Marca (misión, promesa, pilares, personalidad, eslóganes), Identidad Visual (concepto de logo, reglas de uso, variantes), Sistema de Color (paletas primaria/secundaria/acento/semántica con cumplimiento WCAG AA), Tipografía (Outfit/Inter/JetBrains Mono con escala tipográfica completa), Voz y Tono (principios de precisión clínica, frases canónicas anti-alucinación), Iconografía, Sistema de Diseño UI (config de tema Gradio, badges de seguridad, wireframe de layout), Estrategia de Redes Sociales (directrices por plataforma, estrategia de hashtags, pilares de contenido), reglas de Co-Branding, Tokens de Diseño CSS, y estrategia i18n.
- **Implementación Lógica/Técnica:** Se sintetizaron insights de la arquitectura técnica del proyecto (LangGraph multi-agente, pipeline RAG SOTA, política Zero-PHI) en pilares de marca. La paleta de colores se derivó de la estética médica/clínica (teal = confianza clínica, navy = autoridad, ámbar = esperanza). Se definieron propiedades CSS personalizadas como sistema de tokens de diseño para implementación directa en la UI de Gradio. Se estableció la frase canónica anti-alucinación ("Información no concluyente en las guías provistas") como un elemento de marca inmutable. Se crearon versiones bilingües (EN/ES) según el requisito de flujo de trabajo dual.
- **Métricas de Rendimiento:** Manual de marca de 12 secciones entregado. Documentación bilingüe (`.md` + `.es.md`) creada simultáneamente. Sistema completo de tokens CSS listo para integración en UI. Cumplimiento de accesibilidad WCAG 2.1 AA verificado para todas las combinaciones de colores primarios.
